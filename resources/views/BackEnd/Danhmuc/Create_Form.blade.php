  ...
  <div class="card card-primary">
    <form class="col-md-12" method="POST"  enctype="multipart/form-data" >
      {{ csrf_field() }}
        ...
        <input  name="ten_danh_muc"  id="slug" value="{{old('ten_danh_muc')}}" onkeyup="ChangeToSlug()" class="form-control" rows="5">
        @error('ten_danh_muc')
        ...
        <div class="form-group">
          ...
          <input type="text" name="slug_danh_muc"  id="convert_slug" value="{{old('slug_danh_muc')}}"class="form-control" readonly>
         <p class="font-monospace danger">*Slug sẽ được tự động thêm</p>
        </div>
        <div class="form-group">
        <label for="inputSpentBudget">Mô tả</label>
        <textarea type="text"  name="mo_ta" style="height: 110px" class="form-control" size="5">{{old('mo_ta')}}</textarea>
        @error('mo_ta')
          <div class="text-danger">{{$message}}</div>
          @enderror
        </div>
